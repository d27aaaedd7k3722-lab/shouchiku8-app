...
| panels[]（高機能塗装だけの行） | `{"code": "2300", "name": "左Frドア", "method": "高機能"}` | 明細は脱着などで塗り替えないが、その部位に**高機能塗装の加算だけ**足す行（NEO の `PaintingPanel.DisposalCode 7`）。`paint.hf` が「しない」以外のときだけ使える。指数は CHM の高機能列（無ければ F_S / Scrach.DB）から取る。`index` を書けばその値で手入力 `#`。パネル計には入り、加算基礎の枚数には数えない。`reference/painting.md` §13 |
