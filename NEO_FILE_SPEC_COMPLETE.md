...
- PaintingEtcetera（項目ごとに `<項目>=枚数, <項目>Time/TimeStandard, Wage*/WageStandard*`。指数は COM/fukaetc.DB（XOR 0xff CSV、8 行）と 2TONE.DB）:
  ...
  - `TwoCSolidFlag=1, TwoCSolidRoof 0/1, TwoCSolidOther n, TwoCSolidTime` 2コートソリッド（塗膜ソリッド＋塗装条件のチェック）: **基本 0.10 ＋ ルーフ以外 0.10/枚 ＋ ルーフ 0.30**（溶剤 1 行目 `040,010` の 0.40 は「基本 0.10 ＋ ルーフ 0.30」をまとめた値、水性 7 行目 `030,010,010` は [ルーフ, ルーフ以外, 基本] と分かれている。**基本加算はルーフの有無によらず必ず付く** —— 実案件 NEO 153 本すべてと実機（2K: ルーフのみ 0.4 / +1 枚 0.5 / +7 枚 1.1、水性: 1 枚 0.2 / ルーフ+2 枚 0.6）が一致。2026-09-21 に訂正。それまでの式は溶剤でルーフが無いとき 0.10 足りなかった）
...
- **`ERParts.OrderFlag` の値の意味**（実案件 NEO 900 本・明細 47,717 行、2026-09-21）。**金額には影響しない**:

| 値 | 行数 | 何の行か |
|---|---|---|
| `'0'` | 38,290 | ADDATA から引いた品番のまま確定した行（品番あり 32,284・標準価格あり） |
| `'9'` | 4,820 | **品番や価格を人が入れ替えた行**（価格の印 `*` が 4,669、標準価格なし 4,626、品番欄に「社外品/参考価格」等）。汎用車種 Z10 の NEO は全行が `9` |
| `''`（空） | 3,720 | 触っていない行（コグニで新規作成しただけの見積は全行これ。実機 cogni_M1） |
| `'1'/'2'/'3'` | 826/38/3 | **複数候補から選んだ品番の候補番号**。この値の行は品番の末尾が必ず `*` |
| `'*'` | 20 | 候補を選んだ直後（番号が振られる前）。あとの操作で `'1'` 等に変わる |

  生成器は全行 `''` を書く（新規作成と同じ）。AnSMB 100 桁はこの値をそのまま写す（§10-17）
- **`PaintingLinkParts`（塗装連動部品）** は「塗装の対象になりうる明細行の一覧」。実案件 900 本の 2,633 行すべてで、
  **同じ部品コードかつ同じ修理方法の明細行がある**（例外 0）。そのうち塗装パネルになったのは 1,936 行で、残り 697 行は候補のまま
  （`PaintingFlag` はほぼ 0 で、パネルになったかどうかとは無関係）。「(ｺｳﾁﾝ)」の行も候補には入る（150 行中 56 行がパネル）
- **総合計画面の割増・値引き（`Total.pt_Extra*` / `wg_Extra*`）は実務で使われていない**（実案件 900 本で金額の入った NEO が 0 件。
  `pt_ExtraFlag=1` が 250 本あるが金額は 0）。`wg_IncludeMaterial`（値引きに塗装材料代を含む）は 1 が 674 本・0 が 226 本で、
  **値引きが無ければどちらでも金額は同じ**（生成器は 1）
- OrderFlag は工場から届いた NEO を開いて保存したファイルでは '0'、コグニで新規作成した見積では ''（値の意味は上の表。
  品番の候補が複数ある部品は '1'〜'3' で PartsNo の末尾に '*' が付く。実機 2360 ﾄﾞｱﾗｲﾆﾝｸﾞ。ADDATA の暫定フラグとは無関係）
...
- W90 4600 取替に 4800 を足すと 4600 の指数が 4.0 → 11.7（ChangeTotal は 4.0 のまま）。**2026-09-22 に規則を確定**: 本体は区分キーだけで 15.DB を車種全体から引き（DBSEARCH SearchKotin）、その行の host が見積に居れば host、居なければ sub の明細に時間を足す（AnLstBLMng SetConnectPartsTime）。生成器も同じにして W90b は実機と一致（ADDATA 仕様書 §17-3）
...
- 明細で編集した行の `OrderFlag` は `'0'`（= ADDATA の品番のまま確定した行。品番や価格を人が入れ替えた行は `'9'`、触っていない行は `''`。値の意味は §10-17 の表。合計・再評価に影響なし）
...
残った差（金額に無関係・状態依存・未解決）: `ERParts.PartsNo` 末尾 `*` と `OrderFlag='*'`（複数候補ダイアログで選んだ直後。あとの操作で候補番号 `'1'`〜`'3'` に変わる = 実案件 900 本で確定。§10-17）、
`OrderFlag='0'`（明細画面で確定した行。生成器由来は ''。値の意味は §10-17 の表）、~~`ChangeTotal`（4800）= 44,900 + 9.7h×単価~~（**同日に再現**: 表示の Time は -1 のまま、ボディ 20 行 sub 4801 の標準工賃を取替合計に入れる。§10-3-2。未解決で残るのは W90b の 4600 連動加算 +7.7h）、
