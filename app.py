...
【合計額の確認ルール】
明細抽出後、抽出した「部品金額の合計」と「工賃の合計」を計算し、見積書原本に記載されている各合計額（小計等）と一致するか確認してください。
もし一致しない場合は、数値を無理に合わせようとせず、CSVの最終行の下に改行して以下の形式で相違額を記載してください。
部品相違○○○○円
工賃相違●●●●円
（※一致している場合は相違の記載は不要です）
